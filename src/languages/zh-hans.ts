...
（例如：${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER}）`,
    ...
    iou: {
        ...
        rejectedThisReport: (comment?: string) => `已拒绝此报告${comment ? `，备注为 ${comment}` : ''}`,
        ...
        retracted: (memo?: string) => `已撤回${memo ? `，备注为 ${memo}` : ''}`,
        retract: '撤回',
        reopened: '已重新打开',
        reopenReport: '重新打开报表',
...
${amount}，商户：${merchant} - 日期：${date}`,
    ...
    workspace: {
        ...
        qbo: {
            ...
            itemsDescription: '选择如何在 Expensify 中处理 QuickBooks Online 项目。',
