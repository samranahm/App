...
(p.ex., ${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER})`,
    ...
    iou: {
        ...
        rejectedThisReport: (comment?: string) => `rejeitou este relatório${comment ? `, dizendo ${comment}` : ''}`,
        ...
        retracted: (memo?: string) => `retraído${memo ? `, dizendo ${memo}` : ''}`,
        retract: 'Retrair',
        reopened: 'reaberto',
...
Adicione mais regras de gasto para proteger o fluxo de caixa da empresa.`,
    ...
    violations: {
        ...
        inactiveVendor: ({isSupplier = false}: ViolationsInactiveVendorParams = {}) => (isSupplier ? 'Fornecedor não é mais válido' : 'Fornecedor não é mais válido'),
