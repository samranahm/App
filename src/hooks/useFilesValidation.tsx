...
function useFilesValidation(onFilesValidated: (files: FileObject[], dataTransferItems: DataTransferItem[]) => void) {
    ...
    async function validateAndResizeFiles(files: FileObject[], items: DataTransferItem[], validationState: ValidationState) {
        ...
        if (filesToConvert.length > 0) {
            ...
            // One at a time so peak native decode memory stays bounded (same as validation above).
        ...
        if (filesToResize.length > 0) {
            ...
            // One at a time for the same peak-memory reason as HEIC conversion.
            for (const file of filesToResize) {
                try {
                    // eslint-disable-next-line no-await-in-loop
                    const value = await resizeImageIfNeeded(file);
                    validNonPdfFiles.push(value);
                    updateFileOrderMapping(file, value);
                } catch (reason) {
                    const errorMessage = reason instanceof Error ? reason.message : undefined;
